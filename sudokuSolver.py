...
def solvePuzzle(sudoku):
  """ sudoku puzzle solver
        ...
        RETURN: 80 character string of solved puzzle
  """
  ## this is a stolen algorithm rewritten for my comprehension
  global loopCntr
  global solved
  # find first 0 element
  prDebug ( 'DEBUG001:  call:'+ sudoku  )
  loopCntr += 1
  if loopCntr == 2000000: 
    print 'Max Loops Reached: '+str(loopCntr)
    sys.exit(100)
  if '0' in sudoku:
    i = sudoku.index('0')
  else:
    ## nothing more to solve!
    ## this code is depricated since the return path was changed
    ## to more elegantly return from the recursion.
    Done = True
    prDebug ('Solved!')
    prDebug (sudoku)
    return sudoku
  ...
  for j in range(len(sudoku)):
    if inRow(i,j) or inCol(i,j) or inBlk(i,j):
      excludedNums.add(sudoku[j])
      # prDebug( 'DEBUG002: i:' + str(i) + ' ' + sudoku[i] 
      #   + ' j:' + str(j) + ' ' + sudoku[j] 
      #   + ' excludedNums:' + str(excludedNums) )
  # prDebug( 'DEBUG003: i:' + str(i) + ' j:' + str(j) + ' excludedNums:' + str(excludedNums) )
  
  ## Now attempt a substitution
  for trial in '123456789':
    if trial not in excludedNums:
      # prDebug( 'DEBUG004: Trail solution - i:' + str(i) 
      #   + ' trial value:' + str(trial) )
      # prDebug ( 'DEBUG005: trial:'+ sudoku[:i] + trial + sudoku[i+1:] )
      tmpSolution = solvePuzzle( sudoku[:i] + trial + sudoku[i+1:] )
      if '0' not in tmpSolution: 
        if not solved:
          # prDebug('DEBUG009: Solved! loopCntr: '+str(loopCntr))
          prVerbose('SOLVER: Solved! loopCntr: '+str(loopCntr))
          # prDebug(tmpSolution)
          prVerbose(tmpSolution)
        solved = True
        return tmpSolution
  return sudoku
...
def main():
  ...
  startTime = time.time()
  solvedPuzzle = solvePuzzle( sudoku )
  stopTime = time.time()
